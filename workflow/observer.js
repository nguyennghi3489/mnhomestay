/**
 * Workflow Observer - Token usage tracking, phase activity monitoring, and performance analytics
 */
...
export class WorkflowObserver {
  ...
  constructor() {
    ...
    this.multiAgentMetrics = {
      ...
      agentPerformance: new Map([
        ['retrieve', { invocations: 0, successRate: 0, avgDuration: 0, avgTokens: 0, avgCost: 0 }],
        ['generate', { invocations: 0, successRate: 0, avgDuration: 0, avgTokens: 0, avgCost: 0 }],
        ['validate', { invocations: 0, successRate: 0, avgDuration: 0, avgTokens: 0, avgCost: 0 }],
        ['test', { invocations: 0, successRate: 0, avgDuration: 0, avgTokens: 0, avgCost: 0 }]
      ]),
      handoffHistory: []
    };
  ...
  private initializePhasePerformance(): void {
    const phases: RAGPhase[] = ['retrieve', 'augment', 'generate', 'validate', 'test', 'deploy'];
  ...
  private updateAgentPerformance(agent: AgentMetrics): void {
    ...
    const successCount = Math.round(existing.successRate * existing.invocations / 100) + (agent.success ? 1 : 0);

    existing.invocations = newInvocations;
    existing.successRate = (successCount / newInvocations) * 100;
    existing.avgDuration = (existing.avgDuration * (newInvocations - 1) + (agent.duration || 0)) / newInvocations;
    existing.avgTokens = (existing.avgTokens * (newInvocations - 1) + agent.tokensUsed) / newInvocations;
    existing.avgCost = (existing.avgCost * (newInvocations - 1) + agent.cost) / newInvocations;
  }
   ...
   */
  startAgent(agentId: string, phase: RAGPhase, metadata?: Record<string, any>): void {
    if (this.currentAgent) {
      console.warn(`📊 [Observer] Agent ${this.currentAgent.agentId} still running, force completing`);
  ...
  endAgent(success: boolean, error?: string): void {
    ...
    this.currentAgent.duration = this.currentAgent.endTime.getTime() - this.currentAgent.startTime.getTime();
    ...
    console.log(`🤖 [Observer] ${success ? '✅' : '❌'} Agent ${this.currentAgent.agentId} ${success ? 'completed' : 'failed'} (${this.currentAgent.duration}ms)`);
   ...
   */
  recordHandoff(fromAgent: string, toAgent: string, phase: RAGPhase, dataSize: number): void {
    ...
    if (this.multiAgentMetrics.handoffHistory.length > 50) {
      this.multiAgentMetrics.handoffHistory = this.multiAgentMetrics.handoffHistory.slice(-50);
    }

    console.log(`🔄 [Observer] Handoff: ${fromAgent} → ${toAgent} (${dataSize} bytes)`);
  }

  /**
   * Record agent token usage
   */
  recordAgentTokens(inputTokens: number, outputTokens: number, model: string = 'claude-3-sonnet'): void {
    if (!this.currentAgent) {
      console.warn('📊 [Observer] Cannot record agent tokens - no active agent');
      return;
    }
    ...
    console.log(`📊 [Observer] Agent ${this.currentAgent.agentId}: ${totalTokens} tokens, $${cost.toFixed(4)}`);
  }
  recordTokenUsage(
    inputTokens: number, 
    outputTokens: number, 
    ...
    console.log(`📊 [Observer] Tokens: ${inputTokens}→${outputTokens} (${totalTokens} total, $${cost.toFixed(4)})`);
  }
  ...
  private checkTokenAlert(tokens: number, cost: number): void {
    if (tokens > this.alertThresholds.maxTokensPerPhase) {
      console.warn(`🚨 [Observer] High token usage: ${tokens} tokens in single phase`);
    }
    
    if (this.currentIssue && this.currentIssue.totalCost > this.alertThresholds.maxCostPerIssue) {
      console.warn(`🚨 [Observer] High cost issue: $${this.currentIssue.totalCost.toFixed(2)} for ${this.currentIssue.issueId}`);
    }
  }
  ...
  endPhase(status: 'completed' | 'failed' = 'completed', error?: string): void {
    ...
    console.log(`📊 [Observer] Completed phase: ${this.currentPhase.phase} (${this.currentPhase.duration}ms)`);
  ...
  private updatePhasePerformance(phase: PhaseActivity): void {
    ...
    perf.count++;
    perf.avgDuration = ((perf.avgDuration * (perf.count - 1)) + (phase.duration || 0)) / perf.count;
    
    if (phase.tokenUsage) {
      perf.avgTokens = ((perf.avgTokens * (perf.count - 1)) + phase.tokenUsage.totalTokens) / perf.count;
      perf.avgCost = ((perf.avgCost * (perf.count - 1)) + phase.tokenUsage.cost) / perf.count;
    }
  }
   ...
   */
  recordQualityGate(gate: string, passed: boolean, message: string, metrics?: Record<string, number>): void {
    if (!this.currentIssue) {
      console.warn('📊 [Observer] Cannot record quality gate - no active issue');
      return;
    }
  ...
  endIssue(status: 'completed' | 'failed' = 'completed'): void {
    ...
    this.currentIssue.totalDuration = this.currentIssue.endTime.getTime() - this.currentIssue.startTime.getTime();
    ...
    const completedMetrics = Array.from(this.workflowMetrics.issueMetrics.values())
      .filter(m => m.status === 'completed' && m.totalDuration);
    
    this.workflowMetrics.averageIssueTime = completedMetrics.length > 0
      ? completedMetrics.reduce((sum, m) => sum + (m.totalDuration || 0), 0) / completedMetrics.length
      : 0;

    // Check duration alert
    const durationMinutes = (this.currentIssue.totalDuration || 0) / (1000 * 60);
    if (durationMinutes > this.alertThresholds.maxDurationPerIssue) {
      console.warn(`🚨 [Observer] Slow issue: ${this.currentIssue.issueId} took ${durationMinutes.toFixed(1)} minutes`);
    }

    console.log(`📊 [Observer] Completed issue: ${this.currentIssue.issueId} (${durationMinutes.toFixed(1)}min, ${this.currentIssue.totalTokens} tokens, $${this.currentIssue.totalCost.toFixed(4)})`);
  ...
  getWorkflowSnapshot(): {
    ...
    const successRate = this.workflowMetrics.totalIssues > 0 
      ? (this.workflowMetrics.completedIssues / this.workflowMetrics.totalIssues) * 100 
      : 0;

    // Estimate completion time
    const remainingIssues = this.workflowMetrics.totalIssues - this.workflowMetrics.completedIssues - this.workflowMetrics.failedIssues;
    const estimatedCompletion = this.workflowMetrics.averageIssueTime > 0 && remainingIssues > 0
      ? new Date(Date.now() + (remainingIssues * this.workflowMetrics.averageIssueTime))
      : undefined;
    ...
    const currentIssue = this.currentIssue ? {
      ...
      elapsedTime: (Date.now() - this.currentIssue.startTime.getTime()) / (1000 * 60),
      tokensUsed: this.currentIssue.totalTokens,
      costSoFar: this.currentIssue.totalCost
    } : undefined;

    // Phase performance
    const phasePerformance = Array.from(this.workflowMetrics.phasePerformance.entries()).map(([phase, perf]) => ({
      phase,
      count: perf.count,
    ...
    if (this.workflowMetrics.totalCost > 50) {
      alerts.push(`High total cost: $${this.workflowMetrics.totalCost.toFixed(2)}`);
    }
